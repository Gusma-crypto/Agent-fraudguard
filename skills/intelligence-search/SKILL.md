...
optional `entity_type`, and optional `deep_search` as non-secret context. Never request or
submit an OTP, PIN, password, CVV, token, or full credential.
...
For every result, state whether evidence was found. When `sources` or `evidence` is
